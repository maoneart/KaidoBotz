...
const starts = async (Kaido = new WAConnection()) => {
  ...
  Kaido.on("open", () => {
    ...
    console.log(
      color(" ||> ", "cyan"),
      color(`     Hallo Kak ${Miminnya} ${ucapanWaktu}`, "cyan")
    );
    console.log(
      color(" ||> ", "cyan"),
      color("    Jangan Lupa Subscrib Yt KaidoBotz😝", "cyan")
    );
    console.log(
      color(" ||> ", "cyan"),
      color("        Script Ori By Hermawan🛐", "cyan")
    );
    console.log(
      color("", "aqua"),
      color(`||================================================||`, "cyan")
  ...
  fs.writeFileSync(
    "./newbase.json",
    JSON.stringify(Kaido.base64EncodedAuthInfo(), null, "\t")
  );
  ...
  Kaido.on("group-participants-update", async (anu) => {
    ...
    try {
      ...
      if (anu.action == "add") {
        ...
        try {
          ppimg = await Kaido.getProfilePicture(
...
𝘿𝙄 𝙂𝙍𝙐𝙋 𝙄𝙉𝙄`;
        ...
        Kaido.sendMessage(mdata.id, image, MessageType.image, {
          caption: teks,
...
function nocache(module, cb = () => {}) {
  console.log(
    "Module",
    `'${module}'`,
    "Sekarang Sedang Di Awasi Oleh KaidoBotz !!"
  );
